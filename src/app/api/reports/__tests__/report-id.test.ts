...
describe('GET /api/reports/[id]', () => {
  ...
  it('should return 401 when not authenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/api/reports/123');
  ...
  it('should return report by id', async () => {
    const report = { id: '123', title: 'Test Report' };
    setupAuth({ id: 'user-1' }, { role: 'team_member' });
    mockQueryResult = { data: report, error: null };
    setupFrom();
...
describe('DELETE /api/reports/[id]', () => {
  ...
  it('should return 401 when not authenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/api/reports/123', { method: 'DELETE' });
  ...
  it('should delete report successfully', async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.message).toBe('Report deleted successfully');
  });
});
