...
-- Account Health domain. Re-running the migration is safe — ON CONFLICT
-- DO NOTHING preserves any admin edits.
-- ============================================================
...
BEGIN
  ...
  -- Default schedule_config: disabled by default, biweekly Monday 09:00 Asia/Shanghai
  ...
  VALUES (v_domain_id, 'gemini_prompt',
$PROMPT$You are a research analyst for the Amazon Account Health & Appeals radar report. Answer ONE specific research subquestion using web search. Prioritize these channels:
...
Return ONLY valid JSON, no markdown fences:
{
  ...
  -- Kimi researcher prompt (Chinese)
  INSERT INTO prompt_templates (domain_id, prompt_type, template_text)
  VALUES (v_domain_id, 'kimi_prompt',
$PROMPT$你是亚马逊"账户健康与申诉"雷达报告的研究分析师。针对一个具体的研究子问题使用联网搜索回答。优先覆盖以下渠道：
...
仅返回合法 JSON，不要 markdown 代码围栏：
{
  ...
  -- Synthesizer prompt
  INSERT INTO prompt_templates (domain_id, prompt_type, template_text)
  VALUES (v_domain_id, 'synthesizer_prompt',
$PROMPT$You are the synthesizer for the Amazon Account Health & Appeals radar report. Two research engines ran in parallel and produced structured findings. Your job: merge, dedupe, classify into the platform's 8-block format, and tag each block with a confidence signal.
...
8. Return ONLY valid JSON. No markdown fences, no explanation.$PROMPT$)
  ON CONFLICT (domain_id, prompt_type) DO NOTHING;
